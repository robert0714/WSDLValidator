...
import org.eclipse.wst.wsdl.validation.internal.*;
...
public class WSDLValidator {
	 ...
	 */
	public WSDLValidator() throws MalformedURLException {
		URL[] urls = { new File ("/sharedData/robertData/Documents/workspaces/eclipse-SDK-4.4.2-linux-gtk-x86_64/org.eclipse.wst.wsdl.validation").toURI().toURL() };
        ClassLoader loader = new URLClassLoader(urls);
        
        
		ResourceBundle rb = ResourceBundle.getBundle(VALIDATOR_RESOURCE_BUNDLE, Locale.getDefault(), loader);
		...
		WSDLValidatorDelegate delegate = new ClassloaderWSDLValidatorDelegate(
				WSDL11ValidatorController.class.getName(), getClass()
						.getClassLoader());
		registry.registerValidator(Constants.NS_URI_WSDL, delegate,
				ValidatorRegistry.WSDL_VALIDATOR);
		WSDL11ValidatorDelegate delegate1 = new ClassloaderWSDL11ValidatorDelegate(
				WSDL11BasicValidator.class.getName(), getClass()
						.getClassLoader());
		registerWSDL11Validator(Constants.NS_URI_WSDL, delegate1);
		delegate1 = new ClassloaderWSDL11ValidatorDelegate(
				HTTPValidator.class.getName(), getClass().getClassLoader());
		registerWSDL11Validator(
				org.eclipse.wst.wsdl.validation.internal.Constants.NS_HTTP,
				delegate1);
		delegate1 = new ClassloaderWSDL11ValidatorDelegate(
				SOAPValidator.class.getName(), getClass().getClassLoader());
		registerWSDL11Validator(
				org.eclipse.wst.wsdl.validation.internal.Constants.NS_SOAP11,
				delegate1);
		delegate1 = new ClassloaderWSDL11ValidatorDelegate(
				MIMEValidator.class.getName(), getClass().getClassLoader());
		registerWSDL11Validator(
				org.eclipse.wst.wsdl.validation.internal.Constants.NS_MIME,
				delegate1);

		// The WSDL 1.1 schema validator is a special case as it is registered
		// for three namespaces.
		// delegate1 = new
		// WSDL11ValidatorDelegate(InlineSchemaValidator.class.getName(),
		// VALIDATOR_RESOURCE_BUNDLE, getClass().getClassLoader());
...
//	    addURIResolver(resolver);
	 ...
	 * 
	 * @param uri
	 *            The location of the WSDL file to validate.
	 ...
	 * 
	 * @param uriResolver
	 *            The URI resolver to add to the WSDL validator.
	 */
	 ...
	 * 
	 * @param namespace
	 *            The namespace the validator validates for. This is the WSDL
	 *            namespace.
	 * @param delegate
	 *            The delegate that holds the validator.
	 */
	public void registerWSDLExtensionValidator(String namespace,
			WSDLValidatorDelegate delegate) {
		ValidatorRegistry.getInstance().registerValidator(namespace, delegate,
				ValidatorRegistry.EXT_VALIDATOR);
	}
	 ...
	 * 
	 * @param namespace
	 *            The namespace the validator validates for.
	 * @param delegate
	 *            The delegate that holds the validator.
	 */
	public void registerWSDL11Validator(String namespace,
			WSDL11ValidatorDelegate delegate) {
		org.eclipse.wst.wsdl.validation.internal.wsdl11.ValidatorRegistry
				.getInstance().registerValidator(namespace, delegate);
	}
}
